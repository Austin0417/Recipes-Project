...
MainWindow::MainWindow(QWidget *parent)
  ...
  QPixmap backgroundImage(
      ":/button_icons/F:/Pictures/Recipes Background Image.PNG");
  background->setScaledContents(true);
  QPixmap scaledBackground = backgroundImage.scaled(
      this->size(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

  background->setPixmap(scaledBackground);
  background->setAlignment(Qt::AlignCenter);
  background->lower();
  // setStyleSheet("QMainWindow {background-image:
  // url(:/button_icons/F:/Pictures/Recipes Background Image.PNG);
